  ...
  if (symbolTableEntry == NULL) {
    ...
    case SCALAR_TYPE_DESCRIPTOR:
      idNodeAsType->dataType =
          symbolTableEntry->attribute->attr.typeDescriptor->properties.dataType;
      break;
    case ARRAY_TYPE_DESCRIPTOR:
      idNodeAsType->dataType = symbolTableEntry->attribute->attr.typeDescriptor
                                   ->properties.arrayProperties.elementType;
      break;
    }
    //*/
  }
...
int allowRedeclaration(SymbolAttribute *attr1, SymbolAttribute *attr2) {
  if (attr1->attributeKind != TYPE_ATTRIBUTE ||
      attr2->attributeKind != TYPE_ATTRIBUTE)
    return 0;
  ...
  if (prop1->elementType != prop2->elementType)
    return 0;
  if (prop1->dimension != prop2->dimension)
    return 0;
  for (int i = 0; i < prop1->dimension; ++i) {
    if (prop1->sizeInEachDimension[i] != prop2->sizeInEachDimension[i])
      return 0;
  }
  return 1;
}
...
void declareIdList(AST_NODE *declarationNode,
  ...
  while (traverseIDList) {
    ...
    case ARRAY_ID:
      ...
      processDeclDimList(traverseIDList, attribute->attr.typeDescriptor,
      ...
      if (traverseIDList->dataType == ERROR_TYPE) {
        ...
        declarationNode->dataType = ERROR_TYPE;
                     ...
                     .symbolTableEntry->attribute->attr.typeDescriptor->kind ==
                 SCALAR_TYPE_DESCRIPTOR) {
        attribute->attr.typeDescriptor->properties.arrayProperties.elementType =
            typeNode->semantic_value.identifierSemanticValue.symbolTableEntry
                ->attribute->attr.typeDescriptor->properties.dataType;
      } else if (typeNode->semantic_value.identifierSemanticValue
                     .symbolTableEntry->attribute->attr.typeDescriptor->kind ==
        ...
        int typeArrayDimension =
            typeNode->semantic_value.identifierSemanticValue.symbolTableEntry
                ...
                .dimension;
        int idArrayDimension = attribute->attr.typeDescriptor->properties
        ...
        if ((typeArrayDimension + idArrayDimension) > MAX_ARRAY_DIMENSION) {
          ...
          for (indexType = 0, indexId = idArrayDimension;
               ...
               ++indexType, ++indexId) {
            attribute->attr.typeDescriptor->properties.arrayProperties
                .sizeInEachDimension[indexId] =
                typeNode->semantic_value.identifierSemanticValue
                    .symbolTableEntry->attribute->attr.typeDescriptor
                    ->properties.arrayProperties.sizeInEachDimension[indexType];
          }
        }
      }
      break;
    case WITH_INIT_ID:
      if (typeNode->semantic_value.identifierSemanticValue.symbolTableEntry
    ...
    if (traverseIDList->dataType == ERROR_TYPE) {
      free(attribute);
      declarationNode->dataType = ERROR_TYPE;
    } else if (declaredLocally(traverseIDList->semantic_value
                                   .identifierSemanticValue.identifierName)) {
      SymbolTableEntry *entry =
          retrieveSymbol(traverseIDList->semantic_value.identifierSemanticValue
                             .identifierName);
      ...
      if (!allowRedeclaration(declAttr, attribute)) {
        if (attribute->attributeKind == TYPE_ATTRIBUTE)
          printErrorMsg(traverseIDList, CONFLICTING_TYPEDEF);
        else
      ...
      traverseIDList->semantic_value.identifierSemanticValue.symbolTableEntry =
          enterSymbol(traverseIDList->semantic_value.identifierSemanticValue
                          .identifierName,
                      attribute);
      ...
      if (isVariableOrTypeAttribute == VARIABLE_ATTRIBUTE &&
          !ignoreArrayFirstDimSize &&
          !isGlobalVariable(traverseIDList->semantic_value
                                .identifierSemanticValue.symbolTableEntry)) {
...
void checkAssignOrExpr(AST_NODE *assignOrExprRelatedNode) {
  if (assignOrExprRelatedNode->nodeType == STMT_NODE) {
    ...
    } else if (assignOrExprRelatedNode->semantic_value.stmtSemanticValue.kind ==
               FUNCTION_CALL_STMT) {
...
void getExprOrConstValue(AST_NODE *exprOrConstNode, int *iValue,
  ...
  if (exprOrConstNode->nodeType == CONST_VALUE_NODE) {
    ...
    if (exprOrConstNode->dataType == INT_TYPE) {
      ...
      *fValue = exprOrConstNode->semantic_value.exprSemanticValue.constEvalValue
                    .fValue;
    }
  }
}
...
void processExprNode(AST_NODE *exprNode) {
  if (exprNode->semantic_value.exprSemanticValue.kind == BINARY_OPERATION) {
    ...
    if (operand->dataType == INT_PTR_TYPE ||
      ...
      exprNode->dataType = ERROR_TYPE;
    } else if (exprNode->semantic_value.exprSemanticValue.op.unaryOp ==
    ...
    if (typeDescriptor->kind == SCALAR_TYPE_DESCRIPTOR) {
      ...
      if (idNode->dataType != ERROR_TYPE) {
        if (dimension == typeDescriptor->properties.arrayProperties.dimension) {
          ...
          printErrorMsg(idNode, INCOMPATIBLE_ARRAY_DIMENSION);
          idNode->dataType = ERROR_TYPE;
        } else if (typeDescriptor->properties.arrayProperties.elementType ==
                   INT_TYPE) {
          idNode->dataType = INT_PTR_TYPE;
        } else {
          idNode->dataType = FLOAT_PTR_TYPE;
...
void declareFunction(AST_NODE *declarationNode) {
  ...
  if (traverseParameter) {
    ...
    if (traverseParameter->dataType == ERROR_TYPE) {
      ...
      parameter->parameterName =
          parameterID->semantic_value.identifierSemanticValue.identifierName;
      parameter->type = parameterID->semantic_value.identifierSemanticValue
                            .symbolTableEntry->attribute->attr.typeDescriptor;
  ...
  if (!errorOccur) {
    ...
    attribute->offsetInAR = (g_deepestBlockVariableOffset < g_offsetInARAux)
                                ? g_deepestBlockVariableOffset
                                : g_offsetInARAux;
  }
